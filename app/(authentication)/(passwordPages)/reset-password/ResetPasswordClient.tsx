            ...
            <form onSubmit={onSubmit} style={{ display: 'grid', gap: '1rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '0.85rem' }}>
                    <label style={{ display: 'grid', gap: '0.45rem' }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            <button type="button" onClick={() => setShowPassword((current) => !current)} aria-label="Toggle new password visibility" style={{ position: 'absolute', right: '0.9rem', top: '50%', transform: 'translateY(-50%)', zIndex: 1, border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--tp-muted)' }}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button>
